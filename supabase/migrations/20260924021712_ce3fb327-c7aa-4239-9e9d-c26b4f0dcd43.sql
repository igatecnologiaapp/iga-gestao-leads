CREATE OR REPLACE FUNCTION public.create_route_from_plan(
  ...
  _available_minutes integer,
  _start_label text,
  _start_address text,
  _start_latitude double precision,
  _start_longitude double precision,
  _end_label text,
  _end_address text,
  _end_latitude double precision,
  _end_longitude double precision,
  _notes text,
  _lead_ids uuid[]
...
BEGIN
  ...
  _marker := 'Ref. planejamento: ' || _request_key::text;

  PERFORM pg_advisory_xact_lock(hashtextextended(_request_key::text, 0));
  SELECT r.id INTO _existing FROM public.visit_routes r
   WHERE r.created_by = _uid AND r.notes LIKE '%' || _marker || '%' LIMIT 1;
  ...
  INSERT INTO public.visit_routes (
    ...
    end_label, end_address, end_latitude, end_longitude, notes
  ) VALUES (
